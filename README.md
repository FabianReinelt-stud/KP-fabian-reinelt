...
This project shows how the same functional ideas can be expressed differently in Elixir and Go, depending on the language’s design and conventions.

The Elixir implementation follows a typical functional programming style. All data structures are immutable, control flow is expressed through pattern matching, and iteration over token streams is implemented using Enum.reduce/3. Errors are represented explicitly as values, and pipelines are used to express function composition in a declarative way. State changes, such as the output queue and operator stack in the Shunting Yard algorithm, are handled by passing updated values between steps rather than mutating data.

The Go implementation follows a functional design overall, while using imperative constructs internally. Each processing stage is implemented as a pure function with explicit parameters and return values, and no global mutable state is used in the core logic. State, such as stacks, is mutated locally, and errors are propagated explicitly using Go’s conventional (value, error) return style. Iteration is expressed using for loops or a custom Reduce higher-order function.

Although Go does not enforce immutability, the implementation still follows functional principles by keeping state changes local, avoiding hidden side effects, and making the flow of data between stages clear.

Overall, the project shows that functional programming is mainly about design principles rather than language syntax. Elixir provides direct language support for functional concepts, while Go, as a multi-paradigm language, requires more discipline but can still express the same ideas in a clear and idiomatic way.
